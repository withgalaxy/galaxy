...
For SSR deployments, use the standalone adapter with platforms like Docker, Railway, or Fly.io`, c.Output.Type)
	...
	if c.Adapter.Name == AdapterCloudflare && c.Output.Type != OutputStatic {
		return fmt.Errorf(`cloudflare adapter only supports static output

Current configuration:
  output.type = "%s"
  adapter.name = "cloudflare"

Cloudflare Pages does not support Go-based serverless functions. To fix this:
  1. Change output.type = "static" in galaxy.config.toml, OR
  2. Use adapter.name = "standalone" for SSR/hybrid output
